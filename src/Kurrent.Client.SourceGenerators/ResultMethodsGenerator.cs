...
public class ResultMethodsGenerator : ISourceGenerator {
    ...
    static string GenerateSource(INamedTypeSymbol classSymbol, ITypeSymbol successType, ITypeSymbol errorType) {
        ...
        // Constructor
        sb.AppendLine($"{memberIndent}protected {className}(bool isSuccess, {successTypeName} success = default, {errorTypeName} error = default) : base(isSuccess, success, error) {{ }}");
        sb.AppendLine();

        // Static Factory Methods
        sb.AppendLine($"{memberIndent}public new static {minimallyQualifiedClassName} Success({successTypeName} _) => Success(_);");
        sb.AppendLine($"{memberIndent}public new static {minimallyQualifiedClassName} Error({errorTypeName} _) => Error(_);");

        // Implicit Conversion Operators
        sb.AppendLine($"{memberIndent}public static implicit operator {minimallyQualifiedClassName}({successTypeName} _) => Success(_);");
        sb.AppendLine($"{memberIndent}public static implicit operator {minimallyQualifiedClassName}({errorTypeName} _) => Error(_);");

        // Explicit Conversion Operators
        sb.AppendLine($"{memberIndent}public static explicit operator {successTypeName}({minimallyQualifiedClassName} _) => _.AsSuccess;");
        sb.AppendLine($"{memberIndent}public static explicit operator {errorTypeName}({minimallyQualifiedClassName} _) => _.AsError;");
