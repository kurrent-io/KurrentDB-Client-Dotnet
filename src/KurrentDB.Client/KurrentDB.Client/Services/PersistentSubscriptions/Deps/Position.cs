...
/// </summary>
readonly struct Position : IEquatable<Position>, IComparable<Position>, IComparable, IPosition {
	/// <summary>
	...
	public Position(ulong commitPosition, ulong preparePosition) {
		...
		if (commitPosition > long.MaxValue && commitPosition != ulong.MaxValue) throw new ArgumentOutOfRangeException(nameof(commitPosition));

		if (preparePosition > long.MaxValue && preparePosition != ulong.MaxValue) throw new ArgumentOutOfRangeException(nameof(preparePosition));
	...
	public int CompareTo(Position other) => this == other
		? 0
		: this > other
			? 1
			: -1;
	...
	public static bool TryParse(string value, out Position? position) {
		...
		if (!TryParsePosition("C", parts[0], out var commitPosition)) return false;

		if (!TryParsePosition("P", parts[1], out var preparePosition)) return false;
		...
		static bool TryParsePosition(string expectedPrefix, string v, out ulong p) {
			...
			if (prts[0] != expectedPrefix) return false;
