...
public class KurrentClient : IAsyncDisposable {
	...
	internal ValueTask<ServiceClientConnection<T>> Connect<T>(CancellationToken cancellationToken) where T : class =>
		LegacyProxy.Connect<T>(cancellationToken);
...
// 		Settings.ConnectionName ??= $"conn-{Guid.NewGuid():D}";
//
// 		LegacyClusterClient = new(Settings, new());
//
// 		var typeMapper     = new MessageTypeMapper();
// 		var schemaExporter = new SchemaExporter();
// 		var registryClient = new KurrentRegistryClient(Settings);
...
// 	}
//
// 	KurrentDBClientBase LegacyClient { get; }
//
