...
static class DynamicValueMapper {
    public static DynamicValueMap MapToDynamicValueMap(this Metadata source) => new() { Values = { source.MapToDynamicMapField() } };

    public static MapField<string, DynamicValue> MapToDynamicMapField(this Metadata source) =>
        source.Aggregate(
            new MapField<string, DynamicValue>(), (seed, entry) => {
    ...
    public static Metadata MapToMetadata(this MapField<string, DynamicValue> source) =>
        source.Aggregate(
            new Metadata(), (seed, entry) => {
                seed.With(entry.Key, MapToMetadataValue(entry.Value));
                return seed;
            }
        );

    public static Metadata MapToMetadata(this DynamicValueMap source) => MapToMetadata(source.Values);
    ...
    public static bool TryGetValue<T>(this DynamicValueMap source, string key, out T? value) => source.Values.TryGetValue(key, out value);
    ...
    public static T? GetRequiredValue<T>(this DynamicValueMap source, string key) => source.Values.GetRequiredValue<T>(key);
}
