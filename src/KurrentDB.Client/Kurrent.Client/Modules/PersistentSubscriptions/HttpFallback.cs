...
class HttpFallback : IDisposable {
    ...
    internal async Task<T> HttpGetAsync<T>(
        string path, string target, TimeSpan? deadline, UserCredentials? userCredentials,
        Action onNotFound, CancellationToken cancellationToken
    ) {
        var request = CreateRequest(
            path, HttpMethod.Get, target,
            userCredentials
        );
    ...
    internal async Task HttpPostAsync(
        string path, string query, string target, TimeSpan? deadline,
    ...
    HttpRequestMessage CreateRequest(string path, HttpMethod method, string target, UserCredentials? credentials) =>
        CreateRequest(
            path, "", method,
            target, credentials
        );
    ...
    HttpRequestMessage CreateRequest(
        string path, string query, HttpMethod method, string target,
        UserCredentials? credentials
    ) {
        var uriBuilder = new UriBuilder($"{_addressScheme}://{target}") {
            Path  = path,
            Query = query
        };
