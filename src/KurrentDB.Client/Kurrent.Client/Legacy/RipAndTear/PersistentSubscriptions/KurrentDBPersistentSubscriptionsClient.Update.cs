...
partial class KurrentDBPersistentSubscriptionsClient {
    ...
    static UpdateReq.Types.StreamOptions StreamOptionsForUpdateProto(
        string streamName,
    ...
    /// </summary>
    /// <exception cref="ArgumentNullException"></exception>
    internal async Task UpdateToStreamAsync(
        string streamName, string groupName, PersistentSubscriptionSettings settings,
        TimeSpan? deadline = null, UserCredentials? userCredentials = null,
        CancellationToken cancellationToken = default
    ) {
        ...
        if (settings.ConsumerStrategyName is null) throw new ArgumentNullException(nameof(settings.ConsumerStrategyName));
        ...
        using var call = new PersistentSubscriptionsServiceClient(channelInfo.CallInvoker)
            .UpdateAsync(
                new UpdateReq {
...
#pragma warning restore 612
    ...
    /// </summary>
    internal async Task UpdateToAllAsync(
