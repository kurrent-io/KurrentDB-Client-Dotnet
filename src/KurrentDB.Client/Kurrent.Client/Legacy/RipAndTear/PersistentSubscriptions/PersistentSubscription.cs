...
class PersistentSubscription : IDisposable {
        ...
        return (result, enumerator.Current) switch {
            (true, PersistentSubscriptionMessage.SubscriptionConfirmation (var subscriptionId)) =>
                new PersistentSubscription(
                    persistentSubscriptionResult, enumerator, subscriptionId,
                    eventAppeared,
    ...
    async Task Subscribe() {
        ...
        try {
            while (await _enumerator.MoveNextAsync(_cts.Token).ConfigureAwait(false)) {
                if (_enumerator.Current is not PersistentSubscriptionMessage.Event(var resolvedEvent, var retryCount)) continue;
                ...
                if (_enumerator.Current is PersistentSubscriptionMessage.NotFound) {
                    ...
                    SubscriptionDropped(
                        ...
                        new PersistentSubscriptionNotFoundException(_persistentSubscriptionResult.StreamName, _persistentSubscriptionResult.GroupName)
                    );
    ...
    Task NackInternal(Uuid[] ids, PersistentSubscriptionNakEventAction action, string reason) => _persistentSubscriptionResult.Nack(action, reason, ids);
}
