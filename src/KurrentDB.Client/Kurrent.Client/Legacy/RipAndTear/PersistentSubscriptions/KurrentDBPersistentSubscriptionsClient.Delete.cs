...
partial class KurrentDBPersistentSubscriptionsClient {
    ...
    /// Deletes a persistent subscription.
    /// </summary>
    internal async Task DeleteToStreamAsync(
        string streamName, string groupName, TimeSpan? deadline = null,
        UserCredentials? userCredentials = null, CancellationToken cancellationToken = default
    ) {
        ...
        using var call = new PersistentSubscriptionsService.PersistentSubscriptionsServiceClient(channelInfo.CallInvoker)
            .DeleteAsync(
    ...
    public async Task DeleteToAllAsync(
        ...
        await DeleteToStreamAsync(
                SystemStreams.AllStream, groupName, deadline,
                userCredentials, cancellationToken
            )
