...
public class WhateverGenerator : ISourceGenerator {
    ...
    string GetCleanTypeName(ITypeSymbol typeSymbol) {
        ...
        if (string.IsNullOrEmpty(name) || name == "Nullable`1") {
            if (typeSymbol is INamedTypeSymbol nt && nt.OriginalDefinition.SpecialType == SpecialType.System_Nullable_T && nt.TypeArguments.Length == 1) return GetCleanTypeName(nt.TypeArguments[0]) + "Nullable";
