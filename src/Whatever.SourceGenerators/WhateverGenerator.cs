...
using System.Linq;
using System.Text;
// using System.Diagnostics; // For Debugger.Launch
...
namespace Kurrent.Whatever.SourceGenerators
{
    ...
    public class WhateverGenerator : ISourceGenerator
    {
        ...
        private string GetSafeFileNameFromType(ITypeSymbol typeSymbol)
        {
            string simpleName = GetCleanTypeName(typeSymbol); 
            return simpleName
                .Replace("<", "_")
                .Replace(">", "_")
                .Replace(",", "_")
        ...
        private bool ImplementsIWhateverDirectly(INamedTypeSymbol classSymbol, out List<ITypeSymbol> genericTypeArguments)
        {
        ...
        private string GenerateWhateverImplementation(INamedTypeSymbol classSymbol, List<ITypeSymbol> typeArguments)
        {
            ...
            sb.AppendLine($"// <auto-generated by WhateverGenerator for {minimallyQualifiedClassName} (interface-only approach) {DateTime.UtcNow} />");
            ...
            for (int i = 0; i < typeArguments.Count; i++)
            {
                ...
                sb.AppendLine($"{memberIndent}public {typeArgNameForMember} As{propertySuffix}");
                sb.AppendLine($"{memberIndent}{{");
                sb.AppendLine($"{memberIndent}    get");
                sb.AppendLine($"{memberIndent}    {{");
                sb.AppendLine($"{memberIndent}        if (Index != {i}) throw new InvalidOperationException($\"Cannot return as {propertySuffix} as current type is {{Value?.GetType().Name ?? \"unknown\"}} (Index {{Index}})\");");
                sb.AppendLine($"{memberIndent}        return ({typeArgNameForMember})Value!;");
                sb.AppendLine($"{memberIndent}    }}");
                sb.AppendLine($"{memberIndent}}}");
                sb.AppendLine();
            }
            ...
            for (int i = 0; i < typeArguments.Count; i++)
            {
                ...
                bool isTypeArgAlreadyNullable = typeArg.OriginalDefinition.SpecialType == SpecialType.System_Nullable_T;
                
                var nullableSuffix = (isValueType && !isTypeArgAlreadyNullable) ? "?" : "";
                string fullReturnTypeName = $"{explicitReturnTypeName}{nullableSuffix}";

                sb.AppendLine($"{memberIndent}public static explicit operator {fullReturnTypeName}({minimallyQualifiedClassName} whatever)");
                sb.AppendLine($"{memberIndent}{{");
                sb.AppendLine($"{memberIndent}    return whatever.Index == {i} ? whatever.As{propertySuffix} : default({fullReturnTypeName});");
                sb.AppendLine($"{memberIndent}}}");
            }
            sb.AppendLine();
        ...
        private void GenerateEqualityMembers(StringBuilder sb, string indent, string className)
        {
        ...
        private void GenerateSwitchMethods(StringBuilder sb, string indent, List<ITypeSymbol> typeArguments)
        {
        ...
        private void GenerateMatchMethods(StringBuilder sb, string indent, List<ITypeSymbol> typeArguments)
        {
        ...
        private void GenerateTryPickMethods(StringBuilder sb, string indent, List<ITypeSymbol> typeArguments)
        {
            for (int i = 0; i < typeArguments.Count; i++)
            {
        ...
        private string GetCleanTypeName(ITypeSymbol typeSymbol)
        {
            ...
            {
                 if (typeSymbol is INamedTypeSymbol nt && nt.OriginalDefinition.SpecialType == SpecialType.System_Nullable_T && nt.TypeArguments.Length == 1)
                 {
        ...
        private string GetParameterName(ITypeSymbol typeSymbol)
        {
                    ...
                    default:
                        ...
                        if (string.IsNullOrEmpty(cleanTypeName) || cleanTypeName == "_" || cleanTypeName.EndsWith("Nullable"))
                        {
                            if (cleanTypeName.EndsWith("Nullable")) {
                                ...
                                string baseName = cleanTypeName.Substring(0, cleanTypeName.Length - "Nullable".Length);
                        ...
                        else if (char.IsUpper(cleanTypeName[0])) 
                        {
                            ...
                            {
                                bool allUpper = true;
                                foreach(char c_ in cleanTypeName) { if(char.IsLower(c_)) { allUpper = false; break; } }
                                if(allUpper) {
