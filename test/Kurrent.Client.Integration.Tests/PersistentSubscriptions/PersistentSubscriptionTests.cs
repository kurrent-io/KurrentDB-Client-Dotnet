...
public class PersistentSubscriptionTests : KurrentClientTestFixture {
	...
	public async Task subscribes_to_stream(CancellationToken ct) {
		...
		var recordIds = simulation.Game.GameEvents.Select(x => x.RecordId).ToList();
	...
	public async Task subscribe_to_all(CancellationToken ct) {
		...
		// Act
		await AutomaticClient.PersistentSubscription.CreateToAllAsync(
			group,
			eventFilter: StreamFilter.Prefix(streams),
			settings: new PersistentSubscriptionSettings(LogPosition.Earliest),
			ct
		);
