...
public class AppendTests : KurrentClientTestFixture {
    ...
    public async Task appends_message_to_new_stream_with_no_stream_expected_state(CancellationToken ct) {
        ...
        var lastPosition = await AutomaticClient.Streams
            .ReadAll(LogPosition.Latest, 1, Direction.Backwards, ReadFilter.None, HeartbeatOptions.Disabled, ct)
            .FirstAsync(ct).ToResultAsync()
            .ThenAsync(x => Result.Success<LogPosition, Exception>(x.AsRecord.Position))
            .ThrowOnErrorAsync(Should.RecordException);
