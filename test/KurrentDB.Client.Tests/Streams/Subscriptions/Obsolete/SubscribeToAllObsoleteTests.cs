...
using KurrentDB.Client.Tests.Streams;
using KurrentDB.Client.Tests.TestNode;

namespace KurrentDB.Client.Tests;

[Trait("Category", "Target:Streams")]
[Trait("Category", "Operation:Subscriptions")]
[Obsolete("Will be removed in future release when older subscriptions APIs are removed from the client")]
public class SubscribeToAllObsoleteTests(ITestOutputHelper output, SubscribeToAllObsoleteTests.CustomFixture fixture)
	: KurrentTemporaryTests<SubscribeToAllObsoleteTests.CustomFixture>(output, fixture) {
	[Fact]
	public async Task receives_all_events_from_start() {
		...
		foreach (var evt in seedEvents.Take(pageSize))
			await Fixture.Streams.AppendToStreamAsync($"stream-{evt.MessageId.ToGuid():N}", StreamState.NoStream, new[] { evt });
	...
	public async Task receives_all_events_from_position() {
		...
		foreach (var evt in seedEvents.Take(pageSize))
			writeResult = await Fixture.Streams.AppendToStreamAsync($"stream-{evt.MessageId.ToGuid():N}", StreamState.NoStream, new[] { evt });
	...
	public async Task receives_all_filtered_events_from_start(SubscriptionFilter filter) {
		...
		foreach (var evt in seedEvents.Take(pageSize))
			await Fixture.Streams.AppendToStreamAsync($"{streamPrefix}-{evt.MessageId.ToGuid():N}", StreamState.NoStream, new[] { evt });
	...
	public async Task receives_all_filtered_events_from_position(SubscriptionFilter filter) {
		...
		IWriteResult writeResult = new SuccessResult();
		foreach (var evt in seedEvents.Take(pageSize))
			writeResult = await Fixture.Streams.AppendToStreamAsync($"{streamPrefix}-{evt.MessageId.ToGuid():N}", StreamState.NoStream, new[] { evt });
