...
public class PersistentSubscriptionsSerializationTests(ITestOutputHelper output, KurrentPermanentFixture fixture)
	...
	public async Task plain_clr_objects_are_serialized_and_deserialized_using_auto_serialization() {
		...
		var resolvedEvents = await Fixture.Subscriptions.SubscribeToStream(stream, group).Take(2).ToListAsync();
	...
	public async Task
		...
		await using var subscriptionsClient =
			NewSubscriptionsClientWith(serialization => serialization.UseMetadataType<CustomMetadata>());
		...
		var resolvedEvents = await subscriptionsClient.SubscribeToStream(stream, group).Take(2).ToListAsync();
	...
	public async Task append_and_subscribe_to_stream_uses_custom_type_mappings(
		...
		await using var subscriptionsClient =
			NewSubscriptionsClientWith(serialization => customTypeMapping(serialization, "user_registered"));
	...
	public async Task automatic_serialization_custom_json_settings_are_applied() {
		...
		await using var subscriptionsClient =
			NewSubscriptionsClientWith(serialization => serialization.UseJsonSettings(systemTextJsonOptions));
	...
	KurrentPersistentSubscriptionsClient NewSubscriptionsClientWith(
		Action<KurrentClientSerializationSettings> customizeSerialization
	) {
		var settings = Fixture.ClientSettings;
		settings.Serialization = settings.Serialization.Clone();
		customizeSerialization(settings.Serialization);
	...
	async Task<string> CreateToStreamSubscription(string stream, KurrentPersistentSubscriptionsClient? client = null) {
	...
	async Task<string> CreateToAllSubscription(string stream, KurrentPersistentSubscriptionsClient? client = null) {
		string group = Fixture.GetGroupName();
