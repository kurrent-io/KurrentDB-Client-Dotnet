...
public class DeleteTests(ITestOutputHelper output, KurrentPermanentFixture fixture) : KurrentPermanentTests<KurrentPermanentFixture>(output, fixture) {
	...
	public async Task with_timeout_stream_revision_delete_fails_when_operation_expired() {
		...
		var rpcException = await Assert.ThrowsAsync<RpcException>(() => Fixture.Streams.DeleteAsync(stream, new StreamRevision(0), TimeSpan.Zero));
	...
	public async Task with_timeout_stream_revision_tombstoning_fails_when_operation_expired() {
		...
		var rpcException = await Assert.ThrowsAsync<RpcException>(() => Fixture.Streams.TombstoneAsync(stream, new StreamRevision(0), TimeSpan.Zero));
