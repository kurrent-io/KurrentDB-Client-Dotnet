...
public class ResultAsyncExtensionsSyncSourceTests : ResultAsyncExtensionsTestFixture {
    #region . ThenAsync .

    [Test]
    public async Task returns_success_when_sync_source_is_success_and_task_result_binder_returns_success() {
        var                                     source = Result<string, int>.Success(InitialSuccessValue);
    ...
    [Test]
    public async Task returns_error_when_sync_source_is_success_and_task_result_binder_returns_error() {
        var                                     source = Result<string, int>.Success(InitialSuccessValue);
    ...
    [Test]
    public async Task propagates_error_when_sync_source_is_error_and_task_result_binder_is_not_called() {
        var source       = Result<string, int>.Error(DefaultError);
    ...
    [Test]
    public async Task throws_argument_null_exception_when_task_result_binder_is_null_for_sync_source() {
        var                                     source = Result<string, int>.Success(InitialSuccessValue);
    ...
    [Test]
    public async Task returns_success_with_state_when_sync_source_is_success_and_task_result_binder_returns_success() {
        var                                          source = Result<string, int>.Success(InitialSuccessValue);
    ...
    [Test]
    public async Task returns_error_with_state_when_sync_source_is_success_and_task_result_binder_returns_error() {
        var                                          source = Result<string, int>.Success(InitialSuccessValue);
    ...
    [Test]
    public async Task propagates_error_with_state_when_sync_source_is_error_and_task_result_binder_is_not_called() {
        var source       = Result<string, int>.Error(DefaultError);
        var binderCalled = false;
    ...
    [Test]
    public async Task throws_argument_null_exception_when_task_result_binder_with_state_is_null_for_sync_source() {
        var                                          source = Result<string, int>.Success(InitialSuccessValue);
    ...
    [Test]
    public async Task returns_success_when_sync_source_is_success_and_value_task_result_binder_returns_success() {
        var                                          source = Result<string, int>.Success(InitialSuccessValue);
    ...
    [Test]
    public async Task returns_error_when_sync_source_is_success_and_value_task_result_binder_returns_error() {
        var                                          source = Result<string, int>.Success(InitialSuccessValue);
    ...
    [Test]
    public async Task propagates_error_when_sync_source_is_error_and_value_task_result_binder_is_not_called() {
        var source       = Result<string, int>.Error(DefaultError);
        var binderCalled = false;
    ...
    [Test]
    public async Task throws_argument_null_exception_when_value_task_result_binder_is_null_for_sync_source() {
        var                                          source = Result<string, int>.Success(InitialSuccessValue);
    ...
    [Test]
    public async Task returns_success_with_state_when_sync_source_is_success_and_value_task_result_binder_returns_success() {
        var                                               source = Result<string, int>.Success(InitialSuccessValue);
    ...
    [Test]
    public async Task returns_error_with_state_when_sync_source_is_success_and_value_task_result_binder_returns_error() {
        var                                               source = Result<string, int>.Success(InitialSuccessValue);
    ...
    [Test]
    public async Task propagates_error_with_state_when_sync_source_is_error_and_value_task_result_binder_is_not_called() {
        var source       = Result<string, int>.Error(DefaultError);
        var binderCalled = false;
    ...
    [Test]
    public async Task throws_argument_null_exception_when_value_task_result_binder_with_state_is_null_for_sync_source() {
        var                                               source = Result<string, int>.Success(InitialSuccessValue);
    ...
    [Test]
    public async Task returns_mapped_success_when_sync_source_is_success_and_task_mapper_is_used() {
        var                     source = Result<string, int>.Success(InitialSuccessValue);
    ...
    [Test]
    public async Task propagates_error_when_sync_source_is_error_and_task_mapper_is_not_called() {
        var source       = Result<string, int>.Error(DefaultError);
    ...
    [Test]
    public async Task throws_argument_null_exception_when_task_mapper_is_null_for_sync_source() {
        var                     source = Result<string, int>.Success(InitialSuccessValue);
    ...
    [Test]
    public async Task returns_mapped_success_with_state_when_sync_source_is_success_and_task_mapper_is_used() {
        var                          source = Result<string, int>.Success(InitialSuccessValue);
    ...
    [Test]
    public async Task propagates_error_with_state_when_sync_source_is_error_and_task_mapper_is_not_called() {
        var source       = Result<string, int>.Error(DefaultError);
        var mapperCalled = false;
    ...
    [Test]
    public async Task throws_argument_null_exception_when_task_mapper_with_state_is_null_for_sync_source() {
        var                          source = Result<string, int>.Success(InitialSuccessValue);
    ...
    [Test]
    public async Task returns_mapped_success_when_sync_source_is_success_and_value_task_mapper_is_used() {
        var                          source = Result<string, int>.Success(InitialSuccessValue);
    ...
    [Test]
    public async Task propagates_error_when_sync_source_is_error_and_value_task_mapper_is_not_called() {
        var source       = Result<string, int>.Error(DefaultError);
        var mapperCalled = false;
    ...
    [Test]
    public async Task throws_argument_null_exception_when_value_task_mapper_is_null_for_sync_source() {
        var                          source = Result<string, int>.Success(InitialSuccessValue);
    ...
    [Test]
    public async Task returns_mapped_success_with_state_when_sync_source_is_success_and_value_task_mapper_is_used() {
        var                               source = Result<string, int>.Success(InitialSuccessValue);
    ...
    [Test]
    public async Task propagates_error_with_state_when_sync_source_is_error_and_value_task_mapper_is_not_called() {
        var source       = Result<string, int>.Error(DefaultError);
        var mapperCalled = false;
    ...
    [Test]
    public async Task throws_argument_null_exception_when_value_task_mapper_with_state_is_null_for_sync_source() {
        var                               source = Result<string, int>.Success(InitialSuccessValue);
    ...
    [Test]
    public async Task returns_success_from_binder_when_sync_source_is_error_and_task_result_binder_returns_success() {
        var                                     source = Result<string, int>.Error(DefaultError);
    ...
    [Test]
    public async Task returns_error_from_binder_when_sync_source_is_error_and_task_result_binder_returns_error() {
        var                                     source = Result<string, int>.Error(DefaultError);
    ...
    [Test]
    public async Task propagates_success_when_sync_source_is_success_and_task_result_binder_is_not_called() {
        var source       = Result<string, int>.Success(InitialSuccessValue);
        var binderCalled = false;
    ...
    [Test]
    public async Task throws_argument_null_exception_when_task_result_binder_is_null_for_orelse_on_sync_source() {
        var                                     source = Result<string, int>.Error(DefaultError);
    ...
    [Test]
    public async Task returns_success_from_binder_with_state_when_sync_source_is_error_and_task_result_binder_returns_success() {
        var                                          source = Result<string, int>.Error(DefaultError);
    ...
    [Test]
    public async Task returns_error_from_binder_with_state_when_sync_source_is_error_and_task_result_binder_returns_error() {
        var                                          source = Result<string, int>.Error(DefaultError);
    ...
    [Test]
    public async Task propagates_success_with_state_when_sync_source_is_success_and_task_result_binder_is_not_called() {
        var source       = Result<string, int>.Success(InitialSuccessValue);
        var binderCalled = false;
    ...
    [Test]
    public async Task throws_argument_null_exception_when_task_result_binder_with_state_is_null_for_orelse_on_sync_source() {
        var                                          source = Result<string, int>.Error(DefaultError);
    ...
    [Test]
    public async Task returns_success_from_binder_when_sync_source_is_error_and_value_task_result_binder_returns_success() {
        var                                          source = Result<string, int>.Error(DefaultError);
    ...
    [Test]
    public async Task returns_error_from_binder_when_sync_source_is_error_and_value_task_result_binder_returns_error() {
        var                                          source = Result<string, int>.Error(DefaultError);
    ...
    [Test]
    public async Task propagates_success_when_sync_source_is_success_and_value_task_result_binder_is_not_called() {
        var source       = Result<string, int>.Success(InitialSuccessValue);
        var binderCalled = false;
    ...
    [Test]
    public async Task throws_argument_null_exception_when_value_task_result_binder_is_null_for_orelse_on_sync_source() {
        var                                          source = Result<string, int>.Error(DefaultError);
    ...
    [Test]
    public async Task returns_success_from_binder_with_state_when_sync_source_is_error_and_value_task_result_binder_returns_success() {
        var                                               source = Result<string, int>.Error(DefaultError);
    ...
    [Test]
    public async Task returns_error_from_binder_with_state_when_sync_source_is_error_and_value_task_result_binder_returns_error() {
        var                                               source = Result<string, int>.Error(DefaultError);
    ...
    [Test]
    public async Task propagates_success_with_state_when_sync_source_is_success_and_value_task_result_binder_is_not_called() {
        var source       = Result<string, int>.Success(InitialSuccessValue);
        var binderCalled = false;
    ...
    [Test]
    public async Task throws_argument_null_exception_when_value_task_result_binder_with_state_is_null_for_orelse_on_sync_source() {
        var                                               source = Result<string, int>.Error(DefaultError);
    ...
    [Test]
    public async Task returns_mapped_error_when_sync_source_is_error_and_task_mapper_is_used() {
        var                     source = Result<string, int>.Error(DefaultError);
    ...
    [Test]
    public async Task propagates_success_when_sync_source_is_success_and_task_error_mapper_is_not_called() {
        var source       = Result<string, int>.Success(InitialSuccessValue);
        var mapperCalled = false;
    ...
    [Test]
    public async Task throws_argument_null_exception_when_task_error_mapper_is_null_for_sync_source() {
        var                     source = Result<string, int>.Error(DefaultError);
    ...
    [Test]
    public async Task returns_mapped_error_with_state_when_sync_source_is_error_and_task_mapper_is_used() {
        var                          source = Result<string, int>.Error(DefaultError);
    ...
    [Test]
    public async Task propagates_success_with_state_when_sync_source_is_success_and_task_error_mapper_is_not_called() {
        var source       = Result<string, int>.Success(InitialSuccessValue);
        var mapperCalled = false;
    ...
    [Test]
    public async Task throws_argument_null_exception_when_task_error_mapper_with_state_is_null_for_sync_source() {
        var                          source = Result<string, int>.Error(DefaultError);
    ...
    [Test]
    public async Task returns_mapped_error_when_sync_source_is_error_and_value_task_mapper_is_used() {
        var                          source = Result<string, int>.Error(DefaultError);
    ...
    [Test]
    public async Task propagates_success_when_sync_source_is_success_and_value_task_error_mapper_is_not_called() {
        var source       = Result<string, int>.Success(InitialSuccessValue);
        var mapperCalled = false;
    ...
    [Test]
    public async Task throws_argument_null_exception_when_value_task_error_mapper_is_null_for_sync_source() {
        var                          source = Result<string, int>.Error(DefaultError);
    ...
    [Test]
    public async Task returns_mapped_error_with_state_when_sync_source_is_error_and_value_task_mapper_is_used() {
        var                               source = Result<string, int>.Error(DefaultError);
    ...
    [Test]
    public async Task propagates_success_with_state_when_sync_source_is_success_and_value_task_error_mapper_is_not_called() {
        var source       = Result<string, int>.Success(InitialSuccessValue);
        var mapperCalled = false;
    ...
    [Test]
    public async Task throws_argument_null_exception_when_value_task_error_mapper_with_state_is_null_for_sync_source() {
        var                               source = Result<string, int>.Error(DefaultError);
    ...
    [Test]
    public async Task executes_task_action_when_sync_source_is_success() {
        var source         = Result<string, int>.Success(InitialSuccessValue);
    ...
    [Test]
    public async Task does_not_execute_task_action_when_sync_source_is_error() {
        var source         = Result<string, int>.Error(DefaultError);
        var actionExecuted = false;
    ...
    [Test]
    public async Task throws_argument_null_exception_when_task_action_is_null_for_onsuccess_on_sync_source() {
        var             source = Result<string, int>.Success(InitialSuccessValue);
    ...
    [Test]
    public async Task executes_task_action_with_state_when_sync_source_is_success() {
        var source         = Result<string, int>.Success(InitialSuccessValue);
        var actionExecuted = false;
    ...
    [Test]
    public async Task does_not_execute_task_action_with_state_when_sync_source_is_error() {
        var source         = Result<string, int>.Error(DefaultError);
        var actionExecuted = false;
    ...
    [Test]
    public async Task throws_argument_null_exception_when_task_action_with_state_is_null_for_onsuccess_on_sync_source() {
        var                  source = Result<string, int>.Success(InitialSuccessValue);
    ...
    [Test]
    public async Task executes_value_task_action_when_sync_source_is_success() {
        var source         = Result<string, int>.Success(InitialSuccessValue);
        var actionExecuted = false;
    ...
    [Test]
    public async Task does_not_execute_value_task_action_when_sync_source_is_error() {
        var source         = Result<string, int>.Error(DefaultError);
        var actionExecuted = false;
    ...
    [Test]
    public async Task throws_argument_null_exception_when_value_task_action_is_null_for_onsuccess_on_sync_source() {
        var                  source = Result<string, int>.Success(InitialSuccessValue);
    ...
    [Test]
    public async Task executes_value_task_action_with_state_when_sync_source_is_success() {
        var source         = Result<string, int>.Success(InitialSuccessValue);
        var actionExecuted = false;
    ...
    [Test]
    public async Task does_not_execute_value_task_action_with_state_when_sync_source_is_error() {
        var source         = Result<string, int>.Error(DefaultError);
        var actionExecuted = false;
    ...
    [Test]
    public async Task throws_argument_null_exception_when_value_task_action_with_state_is_null_for_onsuccess_on_sync_source() {
        var                       source = Result<string, int>.Success(InitialSuccessValue);
    ...
    [Test]
    public async Task executes_task_action_when_sync_source_is_error() {
        var source         = Result<string, int>.Error(DefaultError);
        var actionExecuted = false;
    ...
    [Test]
    public async Task does_not_execute_task_action_when_sync_source_is_success() {
        var source         = Result<string, int>.Success(InitialSuccessValue);
        var actionExecuted = false;
    ...
    [Test]
    public async Task throws_argument_null_exception_when_task_action_is_null_for_onerror_on_sync_source() {
        var                source = Result<string, int>.Error(DefaultError);
    ...
    [Test]
    public async Task executes_task_action_with_state_when_sync_source_is_error() {
        var source         = Result<string, int>.Error(DefaultError);
        var actionExecuted = false;
    ...
    [Test]
    public async Task does_not_execute_task_action_with_state_when_sync_source_is_success() {
        var source         = Result<string, int>.Success(InitialSuccessValue);
        var actionExecuted = false;
    ...
    [Test]
    public async Task throws_argument_null_exception_when_task_action_with_state_is_null_for_onerror_on_sync_source() {
        var                     source = Result<string, int>.Error(DefaultError);
    ...
    [Test]
    public async Task executes_value_task_action_when_sync_source_is_error() {
        var source         = Result<string, int>.Error(DefaultError);
        var actionExecuted = false;
    ...
    [Test]
    public async Task does_not_execute_value_task_action_when_sync_source_is_success() {
        var source         = Result<string, int>.Success(InitialSuccessValue);
        var actionExecuted = false;
    ...
    [Test]
    public async Task throws_argument_null_exception_when_value_task_action_is_null_for_onerror_on_sync_source() {
        var                     source = Result<string, int>.Error(DefaultError);
    ...
    [Test]
    public async Task executes_value_task_action_with_state_when_sync_source_is_error() {
        var source         = Result<string, int>.Error(DefaultError);
        var actionExecuted = false;
    ...
    [Test]
    public async Task does_not_execute_value_task_action_with_state_when_sync_source_is_success() {
        var source         = Result<string, int>.Success(InitialSuccessValue);
        var actionExecuted = false;
    ...
    [Test]
    public async Task throws_argument_null_exception_when_value_task_action_with_state_is_null_for_onerror_on_sync_source() {
        var                          source = Result<string, int>.Error(DefaultError);
    ...
    [Test]
    public async Task returns_mapped_success_when_sync_source_then_async_map_async_succeeds() {
        var                                     source     = Result<string, int>.Success(InitialSuccessValue);
    ...
    [Test]
    public async Task propagates_error_when_sync_source_then_async_fails_before_map_async() {
        var                                     source          = Result<string, int>.Success(InitialSuccessValue);
        Func<int, Task<Result<string, double>>> thenBinder      = _ => Task.FromResult(Result<string, double>.Error(DefaultError));
    ...
    [Test]
    public async Task returns_success_from_orelse_when_sync_source_then_async_fails_and_orelse_succeeds() {
        var                                        source       = Result<string, int>.Success(InitialSuccessValue);
    ...
    [Test]
    public async Task propagates_error_from_orelse_when_sync_source_then_async_fails_and_orelse_also_fails() {
        var                                          source       = Result<string, int>.Success(InitialSuccessValue);
    ...
    [Test]
    public async Task executes_onsuccess_actions_when_sync_source_chain_succeeds() {
        var source               = Result<string, int>.Success(InitialSuccessValue);
        ...
        Func<int, Task> secondAction = val => // Changed to Task
        {
    ...
    [Test]
    public async Task executes_onerror_action_and_propagates_error_when_sync_source_chain_fails_midway() {
        var source                          = Result<string, int>.Success(InitialSuccessValue);
        ...
        Func<string, Task> onErrorAction = err => // Changed to Task
        {
            err.ShouldBe(DefaultError);
