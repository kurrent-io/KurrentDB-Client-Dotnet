...
public class DeleteStreamSecurityTests(ITestOutputHelper output, SecurityFixture fixture) : KurrentTemporaryTests<SecurityFixture>(output, fixture) {
	...
	public async Task deleting_system_user_stream_with_no_user_is_not_allowed() {
		var streamId = await Fixture.CreateStreamWithMeta(
			streamId: $"${Fixture.GetStreamName()}",
			metadataPermanent: new(acl: new(deleteRole: TestCredentials.TestUser1.Username))
		);
	...
	public async Task deleting_system_admin_stream_with_no_user_is_not_allowed() {
		var streamId = await Fixture.CreateStreamWithMeta(
			streamId: $"${Fixture.GetStreamName()}",
			metadataPermanent: new(acl: new(deleteRole: SystemRoles.Admins))
		);
	...
	public async Task deleting_system_all_stream_with_no_user_is_allowed() {
		var streamId = await Fixture.CreateStreamWithMeta(
			streamId: $"${Fixture.GetStreamName()}",
			metadataPermanent: new(acl: new(deleteRole: SystemRoles.All))
		);
